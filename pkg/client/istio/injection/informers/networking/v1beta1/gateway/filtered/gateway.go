...
func withInformer(ctx context.Context) (context.Context, []controller.Informer) {
	...
	for _, selector := range labelSelectors {
		...
		inf := f.Networking().V1beta1().Gateways()
...
func Get(ctx context.Context, selector string) v1beta1.GatewayInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch knative.dev/eventing-istio/pkg/client/istio/informers/externalversions/networking/v1beta1.GatewayInformer with selector %s from context.", selector)
	}
